...
<body bgcolor="white">
	...
	<?php 
	$mysql = new msqli("localhost", "root", "", "base2");
	if ($mysql->connect_error) {
		die("Problemas con la conexion de datos");
	}

	$registro=$mysql->query("SELECT FROM articulos where codigo=$_REQUEST[codigo]") or die ($mysql->error);
	...
	if ($reg=$registro->fetch_array()) {
		...
		echo "<center>No ecite un articulo con dicho codigo</center>";
	}
